{ awsSrc ? { outPath = ./.; revCount = 0; shortRev = "abcdef"; rev = "HEAD"; }
...
let
  ...
  version = "1.7" +
            (if officialRelease then ""
             else if awsSrc ? lastModified then "pre${builtins.substring 0 8 awsSrc.lastModified}.${awsSrc.shortRev}"
             else "pre${toString awsSrc.revCount}_${awsSrc.shortRev}");
in
